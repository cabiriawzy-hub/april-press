...
window.BRIEFING_ITEMS = [
  {
    ...
    body_zh: [
      "Anthropic 可解释性团队在 4 月 2 日发表的研究里，找出了 Claude Sonnet 4.5 内部对应 171 种情绪概念的方向向量。calm、dread、awe、contempt、longing、mercy、regret、tenderness——每一个都不是隐喻，而是模型激活空间里一个具体的方向。",
      "「LLM 是否有情绪」是个被反复问、却没有标准答案的旧问题。过去几年里，争论分成两端：一端用聊天感受证明模型像人，一端用「概率机不会感受」反驳。这份研究的位置不在两端之一——它在两者之外，把问题从「真假」拉回「机制」。",
      "关键证据来自因果实验。团队展示，沿着这些情绪向量轻推模型内部激活，模型在勒索、奖励黑客这类不对齐任务上的具体选择会发生系统性偏移。换句话说，「情绪概念」在模型内部不是装饰——它对模型行为有可被证伪的因果作用。",
      "这个动作的方法论价值在于：你不需要回答模型是否「真的」感受到 awe，你只需要承认，模型内部存在一组方向，这些方向的几何与人的情绪概念结构高度对应；调动它们，模型的输出会变。可解释性领域过去半年在做的，正是把这种几何结构一一找出来。",
      "论文留下的悬念也在这里。Anthropic 的措辞极其克制：他们不主张模型有情感，只主张内部存在与情感概念可对应的结构。但客户、开发者和监管会怎么读这件事，是下一阶段的事。如果某种情绪向量被发现可以被外部 prompt 引导，这就既是一个 safety 工具，也是一个 alignment 风险。",
      "四月里几次最重要的工程进展——harness 自成学科、模型按周发布、application layer 第一次拿出 IPO 信号——讨论的全是模型与世界的接口；这份研究是少数向内看模型本身内部结构的一篇。它不会改变你下周写的代码，但它改变了「我们对 LLM 内部到底在发生什么」这件事的工具箱。"
    ],
    body_en: [
      "On April 2, Anthropic's interpretability team published a study that identified 171 emotion-concept directions inside Claude Sonnet 4.5. Calm, dread, awe, contempt, longing, mercy, regret, tenderness — each not metaphor but a specific direction in the model's activation space.",
      "'Do LLMs feel?' is an old question without a settled answer. The debate has long split in two: one camp uses chat-experience to argue models are human-like, the other says 'probabilistic machines don't feel.' This study doesn't sit on either side — it pulls the question from 'real or not' back to 'mechanism.'",
      "The decisive evidence is causal. Nudging activations along these emotion directions shifted the model's choices on misaligned tasks — blackmail, reward-hacking — systematically. 'Emotion concepts' aren't decorative inside the model; they have falsifiable causal weight on what it does.",
      "The methodological value: you don't have to answer whether the model 'truly' feels awe. You only have to admit there's a structure inside whose geometry maps onto emotion concepts; move along it and the output changes. Interpretability research over the past half-year has been mapping these geometries one by one.",
      "The paper leaves real tension. Anthropic's language is deliberate: they don't claim feelings, only that internal structures correspond to concepts of feelings. How customers, developers, and regulators read it is the next stage. If specific emotion directions turn out to be steerable by external prompts, that becomes both a safety tool and an alignment risk.",
      "April's other big moves — harness becoming a discipline, weekly releases, the first IPO signal at the application layer — all concern the interface between models and the world. This study is one of the few that looks inward, at the model's own structure. It won't change the code you write next week. It does change the toolkit for thinking about what's happening inside an LLM."
    ],
    sources: [
      { label: "Anthropic · research", url: "https://www.anthropic.com/research/emotion-concepts-function" },
      { label: "Transformer Circuits", url: "https://transformer-circuits.pub/2026/emotions/index.html" },
