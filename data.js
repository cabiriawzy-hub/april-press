...
window.BRIEFING_ITEMS = [
  ...
  },
  {
    ...
    body_zh: [
      "三家实验室在二十二天里把「开源」这个词拆成了三种工程决定。第一枪是 Google DeepMind 在 4 月 2 日推出的 Gemma 4——四个尺寸（E2B / E4B / 26B MoE / 31B Dense）、edge 模型 128K 上下文、大模型最高到 256K、原生视觉与音频，最关键的是 Apache 2.0 协议——无 MAU 限制、无 acceptable use policy、无特殊授权。这是 Gemma 系列史上对商用最开放的一次。",
      ...
      "三种站位的发布节奏也值得记一笔。Gemma 4 走的是大厂推送渠道——Google Cloud / Vertex / 本地端 / Hugging Face 同日齐全；Muse Spark 走的是消费产品发布会节奏——一篇 about.fb.com 公告 + Meta AI 产品默认切换；DeepSeek V4 选了「预览发布」——技术报告 + 权重 + 评测同时给出，把决策权交到独立开发者手里。三种路径告诉你三家公司认为「让谁用这个模型」最重要。",
      ...
      "和卷十二（Decoupled DiLoCo）一起读，开源这一面的图景更完整：DeepSeek 能稳定推出每代权重，背后是过去三年里中文社区在分布式训练 + 长上下文上累积的工程红利；Decoupled DiLoCo 把这条路径的一份关键技术显式公开，相当于把「自己拼训练集群」的可能性进一步放大。",
    ...
    body_en: [
      "Three labs cut the word 'open-source' into three different engineering decisions in twenty-two days. First shot: Google DeepMind's Gemma 4 on April 2 — four sizes (E2B / E4B / 26B MoE / 31B Dense), 128K context on edge models, up to 256K on the larger ones, native vision and audio, and most critically Apache 2.0 — no MAU caps, no acceptable use policy, no special permissions. The most permissive commercial release in Gemma's history.",
      ...
      "Each release pattern is itself worth noting. Gemma 4 ships through big-vendor channels — same-day on Google Cloud / Vertex / on-device / Hugging Face. Muse Spark uses a consumer-product launch cadence — a single about.fb.com post plus the Meta AI default-engine swap. DeepSeek V4 picks 'preview release' — paper, weights, benchmarks together, decision left to independent developers. Three paths reveal three answers to 'who matters most for using this model.'",
      ...
      "Read alongside Volume 12 (Decoupled DiLoCo), the open-source picture rounds out: DeepSeek can ship a generation per cycle because the Chinese-speaking community has accumulated three years of distributed-training and long-context engineering surplus. Decoupled DiLoCo openly publishes a key piece of that path, magnifying 'roll your own training cluster' as a possibility for more labs.",
    ...
    sources: [
      { label: "Google blog · Gemma 4 launch", url: "https://blog.google/innovation-and-ai/technology/developers-tools/gemma-4/" },
      { label: "Google Open Source · Gemmaverse with Apache 2.0", url: "https://opensource.googleblog.com/2026/03/gemma-4-expanding-the-gemmaverse-with-apache-20.html" },
      { label: "Meta · Muse Spark introduction", url: "https://about.fb.com/news/2026/04/introducing-muse-spark-meta-superintelligence-labs/" },
      { label: "TechCrunch · DeepSeek V4 preview", url: "https://techcrunch.com/2026/04/24/deepseek-previews-new-ai-model-that-closes-the-gap-with-frontier-models/" },
  ...
  },
  {
    ...
    body_zh: [
      "The Verge 主编 Nilay Patel 在 4 月 23 日发布长视频文章「The People Do Not Yearn for Automation」，主张：AI 在结构上是不受欢迎的，即便 ChatGPT 的使用率仍在上升。他用「software brain」（软件大脑）描述这道文化分裂线——一种把整个人类经验都装进算法、数据库、循环里的看待世界的方式。",
      ...
      "文章最被传播的句子是结尾那句：「the entire human experience cannot be captured in a database. That's why people hate AI. It flattens them.」——这一段被 Hacker News、Tildes 等社群当作四月最重要的一段技术评论传开。Simon Willison 当天在 blog 上把它放大；Daring Fireball 的 John Gruber 跟着发短文「Beware Software Brain」，把这个隐喻推到主流。",
      "Patel 的这一篇在他播客 Decoder 的同步发布版本上累计 200 万播放——对一个 35 分钟长视频来说是异常高的留存。一个数据点的解读是：这个论题不止是「AI 圈内的反思」，是真正进入了大众科技读者的视野。",
      ...
      "对「采纳率与态度脱钩」这件长期被忽略的事，这一对文章是少有的、直接命名它的尝试。它的政策含义不止是「AI 公司要小心 backlash」——更值得关注的是「监管制定者对 AI 的态度可能与使用率反相关」。当 ChatGPT 月活超过 8 亿时，反对它的政治力量不会减少，反而可能扩大——卷十七（布鲁塞尔的一次未决）是这件事在监管端的第一份脚注。",
      "把 Patel 与 Levie 放在同一周读，方法论上的有趣之处在于：他们都用同一份数据（ChatGPT 使用率上涨）做相反的论证。这件事本身已经是 2026 年 AI 评论的一种新形态——同一组事实可以承载两个完全不同的政策方向，最重要的不再是「数据是什么」，而是「你打算用它论证什么」。"
    ],
    body_en: [
      "On April 23, The Verge's Nilay Patel published a long video-essay 'The People Do Not Yearn for Automation,' arguing that AI is structurally unpopular even as ChatGPT usage continues to climb. He named the cultural fault line 'software brain' — 'a particular way of seeing the world that fits everything into algorithms, databases and loops.'",
      ...
      "The most-quoted line lives in the close: 'the entire human experience cannot be captured in a database. That's why people hate AI. It flattens them.' Hacker News, Tildes, and aggregator forums treated it as April's most-circulated piece of tech criticism. Simon Willison amplified it the same day; Daring Fireball's John Gruber followed with 'Beware Software Brain,' pushing the metaphor into mainstream tech writing.",
      "On Patel's Decoder podcast feed, the synchronized audio version reached 2M plays — unusually high retention for a 35-minute essay. One reading: the thesis didn't stay inside 'AI-circle reflection.' It crossed into mainstream tech readership.",
      ...
      "For the under-named gap between adoption and attitude, this pair is one of the few attempts to name it head-on. The policy implication goes beyond 'AI companies should watch out for backlash' — more importantly, regulator sentiment toward AI may be inversely correlated with usage. When ChatGPT MAU passes 800M, political opposition doesn't shrink; it may grow. Volume 17 (Brussels, undecided) is the first regulatory footnote to this dynamic.",
      "Reading Patel and Levie side by side, the methodological note is: both use the same data (ChatGPT usage rising) to argue opposite policy directions. That alone is already a new shape for 2026 AI commentary — the same set of facts can carry two completely different policy directions, and the load-bearing question is no longer 'what is the data' but 'what are you using it to argue.'"
    ],
    sources: [
      { label: "Simon Willison · summary of Patel piece", url: "https://simonwillison.net/2026/Apr/24/the-people-do-not-yearn-for-automation/" },
      { label: "Daring Fireball · Beware Software Brain", url: "https://daringfireball.net/linked/2026/04/23/patel-software-brain" },
      { label: "Decoder podcast · the people do not yearn", url: "https://open.spotify.com/episode/6daNSP6KkB1IWVGCfehbeh" },
      { label: "Hacker News thread", url: "https://news.ycombinator.com/item?id=47878737" },
  ...
  },
  {
    ...
    body_zh: [
      "Kevin Weil（产品负责人）、Bill Peebles（Sora 技术负责人）、Srinivas Narayanan（企业 CTO）三位高管在 4 月 17 日同日离职；OpenAI for Science 解散重组，旗下 Prism 项目被并回主线；Sora 进入两阶段关停——4 月 26 日下线消费端 web/app、9 月 24 日 API 终止。TechCrunch 把这一周的关键词概括为「shed side quests」——三件事压在一起，是 OpenAI 历史上最大的一次内部收缩。",
      "Sora 的关停理由公开数字给得很硬：每天约亏损 100 万美元的算力成本。这件事在过去两年里是 OpenAI 最大、最贵的「次要押注」——它一直没找到稳定的商业模型，但因为「视频是下一形态」这条叙事强硬地撑着。三月底董事会决定关停，四月底彻底落地。OpenAI 选择用一种相当公开的方式承认：这条叙事到这里。",
      "Weil 是 OpenAI 产品线的实际操盘手，主导 ChatGPT 多个关键功能（Memory、Tasks、Operator）。Peebles 是 Sora 的技术负责人。Narayanan 是企业 CTO。三人同日走，意味着三条主线（消费产品 + Sora 视频 + 企业销售）的人事并没有平滑过渡，而是被一刀切下。这种规模的同步离职在 OpenAI 史上是首次。",
      "OpenAI for Science 是去年宣布的科研项目，旗下旗舰产品 Prism 是「为加速科学发现而生的 AI 平台」。它解散意味着 OpenAI 不再单独维护一条「为科学界做基础研究模型」的产品线——这条线的资源被并回主线模型团队。一年前 Sam Altman 在 Bloomberg 采访里把 Science 列为公司「最值得自豪」的方向；一年后这条线消失，是这个月最具象征意义的转向。",
      ...
      "和卷十三（Google → Anthropic 400 亿）合起来读，2026 年「实验室 + 云」关系网正在被重新切分：Anthropic 加深与 Google + Amazon 双绑定，OpenAI 反向松动 Microsoft 独占。两件事在同一周完成，意味着一个曾经被理解为「OpenAI/Microsoft + Anthropic/AWS」的对立结构，正在变成「每家实验室都在每家云上」的多边市场。",
      "OpenAI 这个月的姿态可以一句话概括：把消费 moonshot（Sora）、科研 moonshot（Science）、产品负责人（Weil）一起放下来，把云独占解开，把 AGI 从叙事中央搬到边缘。它是历史上最克制的一个月——这家公司终于开始减法。"
    ],
    body_en: [
      "Three executives — Kevin Weil (product), Bill Peebles (Sora research lead), and Srinivas Narayanan (enterprise CTO) — departed OpenAI on the same day, April 17. OpenAI for Science dissolved, with its flagship Prism project absorbed back into core teams. Sora entered a two-stage shutdown — consumer web/app sunset April 26, API end-of-life September 24. TechCrunch keyworded the week as 'shed side quests.' Three things compressed into one week add up to the largest internal contraction in OpenAI's history.",
      "Sora's shutdown rationale was made publicly explicit: roughly $1M/day in compute losses. For two years it was OpenAI's biggest, most expensive 'secondary bet' — never finding a stable commercial model, sustained mainly by the 'video is the next form' narrative. Late-March board decision; late-April execution. OpenAI chose to admit, fairly publicly, that the narrative ends here.",
      "Weil ran OpenAI's product line — ChatGPT Memory, Tasks, Operator. Peebles led Sora research. Narayanan was the enterprise CTO. Three in one day means three product lines (consumer product, Sora video, enterprise sales) didn't transition gracefully — they got severed. A simultaneous exit of this scale is unprecedented at OpenAI.",
      "OpenAI for Science was last year's flagship research initiative — Prism, the AI-powered platform 'to accelerate scientific discovery.' Dissolving it means OpenAI no longer maintains a separate 'foundation models for science' product line; resources fold back into the core. A year ago Sam Altman called Science the company's 'most worth being proud of' direction in Bloomberg. A year later it disappears — this month's most symbolic strategic turn.",
      "On April 26, Altman released 'Our Principles' — 1,100 words across five principles. Compared with the 2018 Charter, the word 'AGI' appears 2 times instead of 12; 'we commit to' is replaced with 'we believe.' Not wordplay — the first formal repositioning of the company's own narrative. The old Charter described OpenAI as 'the nonprofit on a mission of AGI safety.' The new Principles shift focus from AGI to 'how we use AI to shape the world' — language of a commercial organization, not a research one.",
      "April 27–28: OpenAI signed an amended Microsoft agreement, ending exclusive licensing (extended to 2032 but non-exclusive). The next day OpenAI models shipped on AWS Bedrock; AWS launched Bedrock Managed Agents running OpenAI's models. Stratechery ran a joint Altman / AWS CEO Matt Garman interview that week — the largest cloud-map shift in years, completed.",
      "Read with Volume 13 (Google → Anthropic, $40B), the 2026 'lab + cloud' relationship is being recut: Anthropic deepens its dual tether to Google + Amazon; OpenAI loosens Microsoft exclusivity. Both completed in the same week. What was understood as 'OpenAI/Microsoft vs Anthropic/AWS' is becoming 'every lab on every cloud' — a multi-lateral market.",
      "OpenAI's posture this month, in one line: drop a consumer moonshot (Sora), drop a research moonshot (Science), drop a product chief (Weil), unlock cloud exclusivity, move AGI from narrative center to margin. Its most restrained month ever — the company finally started subtracting."
    ],
    sources: [
      { label: "TechCrunch · shed side quests", url: "https://techcrunch.com/2026/04/17/kevin-weil-and-bill-peebles-exit-openai-as-company-continues-to-shed-side-quests/" },
      { label: "TheNextWeb · triple exit and pivot", url: "https://thenextweb.com/news/openai-departures-kevin-weil-sora-peebles-enterprise-pivot" },
      { label: "CNBC · executives leave", url: "https://www.cnbc.com/2026/04/17/openai-executives-leave.html" },
      { label: "Bloomberg · former product chief and Sora head", url: "https://www.bloomberg.com/news/articles/2026-04-17/openai-s-former-product-chief-and-sora-head-leave-company" },
      { label: "OpenAI · Our Principles (Sam Altman)", url: "https://openai.com/global-affairs/our-principles/" },
      { label: "Stratechery · Altman + Garman interview", url: "https://stratechery.com/2026/an-interview-with-sam-altman-matt-garman-aws-openai-deal/" }
    ]
  },
  {
    id: "vol-17",
